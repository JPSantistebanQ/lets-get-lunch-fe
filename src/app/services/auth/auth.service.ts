...
export class AuthService {
  ...
  signup(credentials: User): Observable<object> {
    return this.http.post('https://ed-5230661035229184.educative.run:3000/api/users', credentials).pipe(
      mergeMap(res => this.login(credentials))
    );
  }
  login(credentials: User): Observable<object> {
    return this.http.post('https://ed-5230661035229184.educative.run:3000/api/sessions', credentials).pipe(
      map((res: any) => {
